...
| `FPL_INTEL_SMTP_HOST` / `FPL_INTEL_SMTP_PORT` / `FPL_INTEL_SMTP_USER` / `FPL_INTEL_SMTP_PASSWORD` | Yes, to send the double-opt-in reminder confirmation email (issue #79) and Contact Us operator notifications (issue #110) | Read at request time by the live server. **Same variable names and value** as the GitHub Actions secrets below -- one credential pair, reused by the live server and both scheduled workflows, rather than a separate set per consumer. A Gmail account with an [app password](https://myaccount.google.com/apppasswords) works well here (`smtp.gmail.com`, port `587`). |
...
| `FPL_INTEL_RELEASE_NOTES_PAT` (issue #143) | `release-notes.yml`, required for the archival commit's PR to actually open | A fine-grained personal access token (repo-scoped to this repo; Contents + Pull requests, both Read and write), used only for the `gh pr create`/`gh pr merge` calls in the archival step -- confirmed live that the default `github.token` cannot create PRs at all ("GitHub Actions is not permitted to create or approve pull requests"), a separate, repo-wide restriction from the branch ruleset's own PR requirement. Generate at [github.com/settings/personal-access-tokens/new](https://github.com/settings/personal-access-tokens/new); fine-grained tokens require an expiration, so this needs periodic rotation. |
