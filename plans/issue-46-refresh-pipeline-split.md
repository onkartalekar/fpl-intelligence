...
#44/#45; branches on lookup-vs-registered-account) — that prior work
settled the *strategic* shape but not *how* to actually wire
per-request computation into the existing `server.py`/`dashboard.js`
architecture. That's a real implementation-shape question with more
than one reasonable answer, which is what this plan resolves.

**Naming note (2026-08-08):** this doc originally used "guest" for the
no-signup, one-off team-ID lookup this issue builds. That word turned
out to be ambiguous — the user's separate ask for a persistent,
non-OAuth *registered* account (now `plans/issue-27-cloud-hosting.md`'s
"Non-OAuth registered accounts" section, feeding into #44) is a
different thing that also got called "guest" at one point. This doc
now says **"lookup"**/**"visitor"** for the concept #46 actually
builds, and reserves **"registered account"** for the persistent path.
...
"the same for everyone" from "specific to whichever team ID a
visitor supplies," and serving the latter per-request without a full
pipeline re-run.
...
2. **Model-performance history for lookup visitors is structurally unavailable,
   ...
   visitor has no persisted picks history anywhere to backtest against
   — the "how did the model do against what I actually did" view
   simply doesn't apply to a one-off lookup. Worth stating explicitly as
...
4. **A lookup is expensive and unguarded by default.**
...
5. **`ThreadingHTTPServer` spins up an unbounded thread per request.**
   Combined with (4), an *unthrottled* lookup endpoint is a concrete,
   already-verified availability risk today — not a hypothetical
   deferred entirely to #28 — since a burst of lookups can pile up
   slow threads and risks getting the app's own outbound IP
   rate-limited by the official FPL API, degrading the service for
   every visitor, not just the one running lookups.
...
### Which parts of the dashboard actually need the per-request data

Checked every tab's markup (`dashboard.py`) and its render functions
(`dashboard.js`) directly, rather than assuming "Decision Center" is
one unit:

| Tab | Shared or per-team? |
|---|---|
| Preseason overview | Shared |
| Decision Center | Mostly shared (Summary, Profiles, XI + captaincy, Bench & model, Squad & player detail). Only the **Weekly decision** sub-panel (`decision-section-weekly`, literally labeled "Personalized weekly decision" in the markup) is per-team — confirmed against finding 1 above. |
| My Team | Fully per-team |
| My Profile | Different in kind — see below |
| Player Explorer | Shared |
| Fixtures | Shared |
| Transfers & News | Shared |
| Model Performance | Mostly shared (accuracy by horizon, calibration, forecast history). Only **"My team — modeled vs actual"** and **"Player forecast vs actual"** (`renderTeamPerformance`/`renderPlayerPerformance`, confirmed referencing `.manager`) are per-team — and per finding 2, these literally cannot populate for a lookup visitor regardless of API shape, since they need accumulated picks history a one-off lookup never has. |
| Model Status | Shared |

So whichever API shape is chosen only needs to reach **My Team** and
the **Weekly decision** sub-panel — not a whole-tab swap.

**My Profile is out of scope for this issue, and deliberately left
open.** It's the save-settings form (`config/user-profile.json` today),
not a data-viewing tab — for a lookup visitor there's nothing to save
yet (that's #44/#45). What this tab should show for a visitor who
hasn't registered (a read-only note of the team ID they looked up?
hidden entirely?) isn't decided here and shouldn't block #46 — worth a
one-line call-out in the PR rather than silently guessing.

### API shape: how a visitor's team ID reaches the server and results reach the page
...
- **(B) Reload-based, matching the existing house pattern exactly —
  recommended.** The visitor's team ID becomes a `?team_id=` query
  ...
  `state` already containing that visitor's `manager`/`weekly_decisions`,
...
4. **Explicitly out of scope, left to later issues**: lifting the
   ...
   anything resembling model-performance/backtest history for lookup visitors
