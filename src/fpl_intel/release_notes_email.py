...
# Category badge variants (see email_template.py's `_BADGE_VARIANTS`). `Feature` reuses ROLL
# green and `Data` reuses INFO blue -- both already mean "a good/informational outcome" in #83's
# palette, and the same reading holds here. `Fix` deliberately reuses AMBER from #83's stale-data
# notice, not the HIT red also available: a shipped fix is good news for the reader, not a cost
# the way a transfer hit is. `Docs` uses DOCS, a purple not part of #83's original three-color
# triad (#83 never needed a fourth category) -- issue #197 moved its color pair into
# email_template.py alongside the others so it gets a light-mode counterpart too, the same as
# every other badge here. `Chore` reuses SLATE, also added for this email.
...
def _change_card_row_html(change, is_last):
    border_class = "" if is_last else ' class="card-border"'
    border = "" if is_last else f"border-bottom:1px solid {email_template.CARD_BORDER};"
    return (
        f'<tr><td{border_class} style="padding:14px 16px;{border}">'
        f'<p class="text-primary" style="margin:0 0 3px;color:{email_template.TEXT_PRIMARY};'
        'font-size:14px;font-weight:bold;font-family:Arial,Helvetica,sans-serif">'
        f'{email_template.esc(change["title"])}</p>'
        f'<p class="text-muted" style="margin:0;color:{email_template.TEXT_MUTED};font-size:13px;'
        f'line-height:1.5;font-family:Arial,Helvetica,sans-serif">{email_template.esc(change["description"])}</p>'
        '</td></tr>'
    )
...
def _category_section_html(category, changes, top_padding):
    ...
    variant = _CATEGORY_BADGE_VARIANT[category]
    badge = email_template.badge_html(f"{category.upper()} · {len(changes)}", variant)
    rows = "".join(
    ...
    card = (
        ...
        f'class="card-bg card-border" style="background:{email_template.CARD_BG};'
        f'border:1px solid {email_template.CARD_BORDER};border-radius:8px">{rows}</table>'
    )
    return (
...
def _section_html(heading_html, changes_by_category, divider_above):
    ...
    divider = (
        ...
        f'<div class="card-border" style="border-top:1px solid {email_template.CARD_BORDER}"></div></td></tr>'
        if divider_above else ""
    )
    heading = (
        f'<tr><td style="padding:20px 16px 0 16px">'
        f'<p class="text-primary" style="margin:0;color:{email_template.TEXT_PRIMARY};font-size:13px;'
        'font-weight:bold;letter-spacing:.4px;font-family:Arial,Helvetica,sans-serif;'
        f'text-transform:uppercase">{heading_html}</p></td></tr>'
    )
...
def _release_notes_footer_html(unsubscribe_url):
    return (
        ...
        f'border-top:1px solid {email_template.CARD_BORDER}">'
        f'<p class="text-muted" style="font-size:11px;color:{email_template.TEXT_MUTED};margin:16px 0 0;'
        ...
        f'<a href="{email_template.esc(unsubscribe_url)}" class="text-muted" '
        f'style="color:{email_template.TEXT_MUTED};text-decoration:underline">Unsubscribe</a></p></td></tr>'
    )
...
def _compose_release_notes_html_body(entry, unsubscribe_url):
    ...
    header_badge = email_template.badge_html(
        f"WHAT'S NEW · {_format_release_notes_badge_date(entry['date'])}", "info",
    )
    header_html = (
        f'<tr><td style="padding:16px 16px 0 16px">{header_badge}</td></tr>'
        f'<tr><td style="padding:14px 16px 0 16px">'
        f'<h1 class="text-primary" style="margin:0;color:{email_template.TEXT_PRIMARY};font-size:20px;'
        f'line-height:1.35;font-family:Arial,Helvetica,sans-serif">{email_template.esc(entry["headline"])}</h1>'
        '</td></tr>'
        f'<tr><td style="padding:8px 16px 0 16px">'
        f'<p class="text-muted" style="margin:0;color:{email_template.TEXT_MUTED};font-size:14px;'
        f'line-height:1.55;font-family:Arial,Helvetica,sans-serif">{email_template.esc(entry["summary"])}</p>'
        '</td></tr>'
    ...
    under_the_hood_heading = (
        'Under the hood <span class="text-muted" style="color:%s;font-weight:normal;'
        'text-transform:none;letter-spacing:normal">(for the developer in you)</span>'
        % email_template.TEXT_MUTED
    )
